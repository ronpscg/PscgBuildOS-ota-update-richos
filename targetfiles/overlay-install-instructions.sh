...
if [[ "$dstdir" =~ fsmaterials/upper ]] ; then 
	...
	dstdir=$dstdir/../merged
	echo "Demonstrating an overlay chroot trick (could just create a symlink instead...)" | tee /dev/console
fi

if [ -d $dstdir/etc/systemd/system/multi-user.target.wants/ ] ; then	
	chroot $dstdir sh -c "systemctl enable ota" || { echo "Failed to enable the OTA service" | tee /dev/console ; exit 1; }
else
